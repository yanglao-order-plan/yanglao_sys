...
import { type IGetOrderData,IWeightData,IArgData,IFlowData} from "@/api/order_detection/types/order_detection"
...
const handleDetect = () => {
  getDetectDataApi(Number(selectedOrderData.orderId))
    .then((res) => {
      // 清空 detectData，防止追加重复数据
      detectData.splice(0, detectData.length);
