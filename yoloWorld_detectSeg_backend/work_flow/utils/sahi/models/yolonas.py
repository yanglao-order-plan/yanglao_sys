...
from work_flow.utils.sahi.models.base import (
    DetectionModel,
)
from work_flow.utils.sahi.prediction import (
    ObjectPrediction,
)
from work_flow.utils.sahi.utils.compatibility import (
    fix_full_shape_list,
...
class YoloNasDetectionModel(DetectionModel):
    def __init__(
        ...
        **kwargs,
        ...
        if (
            not model_path
        ):  # use pretrained flows downloaded from Deci-AI remote client
            self.pretrained_weights = "coco"
            self.class_names = None
            self.num_classes = None
        else:  # use local / custom trained flows
            self.pretrained_weights = None
            if not class_names_yaml_path:
                raise ValueError(
                    "'class_names_yaml_path' should be provided for the flows that have custom class mapping"
                )
    ...
    def set_model(self, model: Any):
        ...
        if self.class_names:  # override class names for custom trained flows
