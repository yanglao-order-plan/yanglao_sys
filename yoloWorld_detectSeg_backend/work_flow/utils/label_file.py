...
from .shape import Shape, rectangle_from_diagonal
...
class LabelFile:
    ...
    def load_image_file(filename, default=None):
        try:
            ...
            print_red(f"Failed opening image file: {filename}")
            return default
                ...
                for i in range(len(data["shapes"])):
                    ...
                    if shape_type == "rectangle" and len(shape_points) == 2:
                        ...
                        data["shapes"][i]["points"] = (
                            rectangle_from_diagonal(shape_points)
                        )
    ...
    def _check_image_height_and_width(image_data, image_height, image_width):
        img_arr = img_b64_to_arr(image_data)
