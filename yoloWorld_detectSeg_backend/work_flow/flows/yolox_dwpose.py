...
from . import  __preferred_device__, Shape, Model, AutoLabelingResult, inference_pose
    ...
    def __init__(self, model_config, on_message) -> None:
        ...
        if not det_model_abs_path or not os.path.isfile(det_model_abs_path):
            raise FileNotFoundError(
                "Could not download or initialize YOLOX-L model."
            )
        ...
        if not pose_model_abs_path or not os.path.isfile(pose_model_abs_path):
            raise FileNotFoundError(
                "Could not download or initialize DWPose model."
            )
    ...
    def predict_shapes(self, image, image_path=None):
        ...
        for box, _, cls_inds, kpt_points, kpt_scores in zip(
            ...
            for i in range(num_kpts):
                ...
                point_shape.add_point(kpt_point[0], kpt_point[1])
