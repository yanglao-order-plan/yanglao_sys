...
from . import __preferred_device__, Model, AutoLabelingResult, Shape
...
class CLRNet(Model):
    ...
    def __init__(self, model_config, on_message) -> None:
        ...
        if not model_abs_path or not os.path.isfile(model_abs_path):
            raise FileNotFoundError(
                "Could not download or initialize CLRNet model."
            )
    ...
    def predict_shapes(self, image, image_path=None):
        ...
        for i, info in enumerate(infos):
            ...
            shape.add_point(start_point[0], start_point[1])  # Replaced QtCore.QPointF with tuple
            shape.add_point(end_point[0], end_point[1])  # Replaced QtCore.QPointF with tuple
            shapes.append(shape)
